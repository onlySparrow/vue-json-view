...
<template>
	<div>
		...
		<span v-else class="tree-view-item-value" :class="[getValueType(data),[iserror?'errorColor':'']]" v-html="valueFormed"></span>
...
<script>
	...
	export default {
		name: 'tree-view-item',
		props: ['data', 'modifiable', 'key-string', 'link', 'errorword'],
		data: function() {
			return {
		...
		methods: {
			...
			getValue: function(value) {
				...
				if(_.isString(value)) {
					let result = "\"" + value + "\"";
					let res;
					if(this.errorword && this.errorword.length > 0) {
						let erroeLen = this.errorword.length;
						let nLen = parseInt(erroeLen) + 2;
						if(result.indexOf(this.errorword) != -1) {
							this.iserror = true;
							res = result.substring(nLen);
						} else {
							this.iserror = false;
							res = result;
						}
